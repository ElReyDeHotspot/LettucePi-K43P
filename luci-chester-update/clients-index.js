...
'require uci';
'require dom';
'require misectel.ui as misectel';
'require misectel.clients as clients';
...
const STYLE = `
.chester-clients .misectel-device-table { table-layout: fixed; width: 100%; }
.chester-clients .misectel-device-table th,
.chester-clients .misectel-device-table td {
...
.chester-clients .misectel-table-input { width: 100%; box-sizing: border-box; }
...
.chester-clients .misectel-device-table__add-row .cl-main,
.chester-clients .misectel-device-table__add-row .cl-sub {
	overflow: visible;
	min-height: 0;
	opacity: 1;
	font-size: 100%;
}
.chester-clients .misectel-device-table__add-row .cl-sub { margin-top: .35em; }
...
return view.extend({
	...
	async load() {
		misectel.ensureBaseStyle();
		await uci.load([ 'dhcp', 'misectel' ]);
	},
	...
	buildDevices(data) {
		...
		devices.forEach((device) => {
			const lease = leases[misectel.normalizeMac(device.mac)];
	...
	async saveClientName(mac, rawName) {
		...
		const leases = this.buildStaticLeaseMap();
		const existing = leases[misectel.normalizeMac(mac)];
		...
		await misectel.saveAndApplyUnchecked(uci);
	},

	openRenameModal(device) {
		const current = device.customName || '';
		const input = E('input', {
			'class': 'misectel-input',
			'type': 'text',
			'value': current,
			'placeholder': _('e.g. Living-Room-TV')
		});
		const preview = E('div', { 'class': 'misectel-device-row__sub' }, [ ' ' ]);
		...
		const save = misectel.createButton(_('Save & Apply'), 'misectel-button',
			ui.createHandlerFn(this, async function (ev) {
				misectel.setButtonBusy(ev.currentTarget, true);
				try {
					await this.saveClientName(device.mac, input.value);
					ui.hideModal();
					window.setTimeout(() => window.location.reload(), 300);
				} catch (err) {
					misectel.notify(_('Could not save the name: %s').format(err.message || err), 'error');
				} finally {
					misectel.setButtonBusy(ev.currentTarget, false);
				}
			}));

		const buttons = [ misectel.createButton(_('Cancel'), 'misectel-button-secondary', () => ui.hideModal()) ];
		if (current)
			buttons.push(misectel.createButton(_('Clear Name'), 'misectel-button-danger',
				ui.createHandlerFn(this, async function (ev) {
					misectel.setButtonBusy(ev.currentTarget, true);
					try {
						...
						misectel.notify(_('Could not clear the name: %s').format(err.message || err), 'error');
					} finally {
						misectel.setButtonBusy(ev.currentTarget, false);
					}
				})));
		...
		ui.showModal(_('Name This Device'), [
			E('div', { 'class': 'misectel-card' }, [
				E('div', { 'class': 'misectel-form-grid' }, [
					misectel.createField(_('Name'), input),
					misectel.createField(_('MAC Address'),
						E('input', { 'class': 'misectel-input', 'type': 'text', 'value': device.mac, 'readonly': 'readonly' }))
				]),
				preview,
				E('p', { 'class': 'misectel-card__desc' }, [
				...
				E('div', { 'class': 'misectel-card__footer' }, buttons)
			])
		], 'cbi-modal');
	},
	...
	async saveStaticLease(name, mac, ipaddr) {
		...
		const sid = staticLeases[misectel.normalizeMac(mac)]?.section || uci.add('dhcp', 'host');
	...
	openStaticLeaseModal(device) {
		const nameInput = E('input', { 'class': 'misectel-input', 'type': 'text',
			'value': device.customName || (device.hostname !== '--' ? device.hostname : '') });
		const ipInput = E('input', { 'class': 'misectel-input', 'type': 'text',
			'value': device.ipaddr !== '--' ? device.ipaddr : '' });
		const macInput = E('input', { 'class': 'misectel-input', 'type': 'text', 'value': device.mac, 'readonly': 'readonly' });
		const interfaceInput = E('input', { 'class': 'misectel-input', 'type': 'text', 'value': 'lan', 'readonly': 'readonly' });

		const saveButton = misectel.createButton(_('Save & Apply'), 'misectel-button',
			ui.createHandlerFn(this, async function (ev) {
				misectel.setButtonBusy(ev.currentTarget, true);
				try {
					...
					misectel.notify(_('Failed to save static IP binding: %s').format(err.message || err), 'error');
				} finally {
					misectel.setButtonBusy(ev.currentTarget, false);
				}
			}));
		...
		ui.showModal(_('Add Static IP'), [
			E('div', { 'class': 'misectel-card' }, [
				E('div', { 'class': 'misectel-form-grid' }, [
					misectel.createField(_('Name'), nameInput),
					misectel.createField(_('IPv4 Address'), ipInput),
					misectel.createField(_('MAC Address'), macInput),
					misectel.createField(_('Interface'), interfaceInput)
				]),
				E('div', { 'class': 'misectel-card__footer' }, [
					misectel.createButton(_('Cancel'), 'misectel-button-secondary', () => ui.hideModal()),
					saveButton
				])
			])
		], 'cbi-modal');
	...
	async removeStaticLease(device) {
		...
		const removeButton = misectel.createButton(_('Remove & Apply'), 'misectel-button-danger',
			ui.createHandlerFn(this, async function (ev) {
				misectel.setButtonBusy(ev.currentTarget, true);
				try {
					...
					misectel.notify(_('Failed to remove static IP binding: %s').format(err.message || err), 'error');
				} finally {
					misectel.setButtonBusy(ev.currentTarget, false);
				}
			}));
	...
	statusDetail(device) {
		...
		if (online && device.connection === 'wireless')
			return `${device.signal}${misectel.hasValue(device.ssid) ? ` / ${device.ssid}` : ''}`;
	...
	renderDeviceRow(device) {
		...
		const actions = [
			misectel.createButton(device.customName ? _('Rename') : _('Name'),
				'misectel-button-secondary', () => this.openRenameModal(device))
		];
		if (device.staticLease && device.hasStaticIp)
			actions.push(misectel.createButton(_('Remove Static IP'), 'misectel-button-secondary',
				() => this.removeStaticLease(device)));
		else
			actions.push(misectel.createButton(_('Add Static IP'), 'misectel-button-secondary',
		...
		return E('tr', {}, [
			...
			cell(misectel.createBadge(online ? _('Online') : _('Offline'), device.tone),
	...
	renderAddStaticLeaseRow() {
		...
		const nameInput = E('input', { 'class': 'misectel-input misectel-table-input', 'type': 'text', 'placeholder': _('Name'), 'value': draft.name });
		const macInput = E('input', { 'class': 'misectel-input misectel-table-input', 'type': 'text', 'placeholder': _('MAC Address'), 'value': draft.mac });
		const ipInput = E('input', { 'class': 'misectel-input misectel-table-input', 'type': 'text', 'placeholder': _('IPv4 Address'), 'value': draft.ip, 'inputmode': 'numeric' });
		...
		const saveButton = misectel.createButton(_('Add Static IP'), 'misectel-button',
			ui.createHandlerFn(this, async function (ev) {
				updateDraft();
				misectel.setButtonBusy(ev.currentTarget, true);
				try {
		...
		return E('tr', { 'class': 'misectel-device-table__add-row' }, [
	...
	renderDeviceList(devices) {
		if (!devices.length)
			return misectel.createEmptyState(
		...
		return E('div', { 'class': 'misectel-device-table-wrap' }, [
			E('table', { 'class': 'misectel-table misectel-device-table' }, [
	...
	renderHeroMeta(data, devices) {
		...
		const pill = (label, value) => E('span', { 'class': 'misectel-pill' }, [
	...
	createShell() {
		this.heroMetaNode = E('div', { 'class': 'misectel-hero__meta' });
		this.listNode = E('div');
		this.loadingNode = E('div', { 'class': 'misectel-loading' }, [ _('Refreshing online devices...') ]);
		...
		ensureStyle();
		return E('div', { 'class': 'misectel-page chester-clients' }, [
			E('div', { 'class': 'misectel-shell' }, [
				E('section', { 'class': 'misectel-hero' }, [
					E('div', { 'class': 'misectel-hero__content' }, [
						E('h2', { 'class': 'misectel-hero__title' }, [ _('Device Management') ]),
						E('div', { 'class': 'misectel-hero__subtitle' }, [
							_('Click a device name to label it. Names are kept even when the device is offline.')
						]),
						this.heroMetaNode
					])
				]),
				misectel.createCard(_('Client List'),
	...
	render() {
		...
		return this.fetchData().then((data) => {
			...
			dom.content(this.listNode, [
				misectel.createEmptyState(_('Failed to load online devices'), _('Please try again later.'))
			]);
			return shell;
		});
	},
