(function () {
	...
	function findCard(label) {
		var cards = document.querySelectorAll('.misectel-page--wifi .misectel-grid > .misectel-card, .misectel-page--wifi .misectel-grid > section');
	...
	function add(uci, card, radio, maximum) {
		...
		var field = el('div', 'misectel-field wifi-txpower-field');
		var head = el('div', 'misectel-field__head');
		var control = el('div', 'misectel-field__control');
		var wrap = el('div', 'misectel-select-wrap');
		var select = el('select', 'misectel-select');
		var configured = uci.get('wireless', radio, 'txpower') || '';
		field.dataset.wifiTxpower = radio;
		head.appendChild(el('label', 'misectel-field__label', 'Transmit Power'));
		...
		(card.querySelector('.misectel-card__body') || card).appendChild(field);
	}
...
(function () {
	...
	function enableFields() {
		return Array.prototype.filter.call(document.querySelectorAll('.misectel-page--wifi .misectel-field'), function (field) {
			if (field.classList.contains('wifi-top-control')) return false;
			var head = field.querySelector('.misectel-field__label');
	...
	function install() {
		var outer = document.querySelector('.misectel-page--wifi .misectel-shell > .misectel-card');
		...
		var modeField = Array.prototype.find.call(outer.children, function (child) {
			return child.classList && child.classList.contains('misectel-field') && child.querySelector('input[type=checkbox]');
		});
		if (!modeField) return false;
		var modeSource = modeField.querySelector('input[type=checkbox]');
		var outerHead = outer.querySelector(':scope > .misectel-card__head');
		if (outerHead) {
			var outerTitle = outerHead.querySelector('.misectel-card__title');
			var outerDescription = outerHead.querySelector('.misectel-card__description');
